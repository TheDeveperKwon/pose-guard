...
- 여러 사람이 잡히는 상황에서 주요 사용자 1인 추적 기반 판정
- 모니터링 시작 시 자동 기준 자세(Baseline) 캘리브레이션
- 재캘리브레이션 진행률(%) 및 상태 메시지 표시
- 자세 경고 상태가 일정 시간(기본 2초) 지속될 때만 알림음 재생(디바운스)
- 매너모드: 공공장소에서 소리 대신 화면 가장자리 시각 알림
- 카메라 미리보기 기본 OFF(프라이버시 중심), 판정 결과 중심 UI
- 단일 Start/Stop 버튼 + 설정 패널 접기/펼치기 + 항목별 툴팁
- 첫 실행 온보딩(1회) 제공
...
2. 얼굴/어깨가 화면에 보이도록 정면 자세를 유지해 자동 캘리브레이션 진행
3. 상태 패널에서 `Turtle Neck`, `Slouching`, `Text Neck` 확인
4. 필요 시 `Settings`를 열어 민감도/매너모드/볼륨/절전모드 조정
5. 기준 자세를 다시 잡고 싶으면 `Settings > Recalibrate Baseline` 클릭
6. 중지 시 동일 버튼(`Stop Monitoring`) 클릭
...
- 캘리브레이션이 완료되기 전에는 자세 경고 판정이 시작되지 않습니다.
...
- [x] Stage 1: 여러 사람 감지 시 주요 사용자 필터링
  - [x] `MediaPipeAdapter.js`: 후보 포즈 수집 후 가시성 + 크기 + 화면 중심도 + 이전 프레임 anchor 기반으로 1인 선택
  - [x] 인원 미검출 시 `null` 반환
- [x] Stage 2: 카메라 기본 OFF + 자동 캘리브레이션
  - [x] `MonitorService.js`: 시작 시 자동 캘리브레이션, 미검출 시 `No User Detected` 처리, 유효 자세까지 캘리브레이션 유지
  - [x] `view.js`, `index.html`: 카메라 기본 OFF, 판정 결과 중심 기본 화면
- [x] Stage 3: UI/UX 단순화 + 설정 숨김 + 호버 설명
  - [x] `index.html`: 단일 Start/Stop 버튼, Settings 접기/펼치기, Recalibrate Baseline 이동, 툴팁 추가
  - [x] `styles.css`: 새 레이아웃/버튼/접힌 설정/툴팁/상태 UI 반영
  - [x] `view.js`: 단일 토글 버튼 로직, 설정 패널 토글, 통계 초기화 및 기본 상태 초기화 반영
- [x] Stage 4: 첫 실행 온보딩 1화면 + 캘리브레이션 진행률 표시
  - [x] 첫 실행 1회 온보딩 모달(로컬 저장 기반 재노출 방지)
  - [x] 재캘리브레이션 진행률(%) 및 안내 메시지 UI 표시
