...
- `Visual Alert` (기본 ON): 전체 화면 가장자리 글로우 오버레이 표시
- `Sound Alert` (기본 OFF): 필요 시 소리 알림 추가 활성화
...
4. 필요 시 `Settings`에서 민감도/시각 알람/소리 알람/절전모드/볼륨 조정
...
- `Sound Alert`를 함께 켜면 동일 조건에서 소리 알림도 추가로 재생됩니다.
